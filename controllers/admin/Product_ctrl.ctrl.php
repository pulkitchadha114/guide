...
class Product_ctrl
{
    ...
    public function list($req = null)
    {
        ...
        $total_page = $this->product_list(ord: "DESC", limit: 10000, active: 1);
        $tp = count($total_page);
        if ($tp %  $data_limit == 0) {
            $tp = $tp / $data_limit;
        } else {
        ...
        if (isset($req->search)) {
            $product_list = $this->product_search_list($keyword = $req->search, $ord = "DESC", $limit = $page_limit, $active = 1);
        } else {
            $product_list = $this->product_list(ord: "DESC", limit: $page_limit, active: 1);
        }
        $context = (object) array(
    ...
    public function trash_list($req = null)
    {
        ...
        if (isset($req->search)) {
            $product_list = $this->product_search_list($keyword = $req->search, $ord = "DESC", $limit = $page_limit, $active = 0);
        } else {
            $product_list = $this->product_list(ord: "DESC", limit: $page_limit, active: 0);
        }
        $context = (object) array(
            'page' => 'products/list.php',
            'data' => (object) array(
    ...
    // Post list
    public function product_list($ord = "DESC", $limit = 5, $active = 1, $sort_by = 'views')
    {
    ...
    public function product_search_list($keyword, $ord = "DESC", $limit = 5, $active = 1)
    {
        $cntobj = new Model('content');
